...
(async () => {
  ...
  if ((await c.query("select 1 from persons limit 1")).rowCount) {
    console.error("persons already seeded — aborting.");
    await c.end();
    process.exit(1);
  }
  const vuvales = (
    await c.query(
      "select id,label from scope_nodes where level='vuvale' order by label"
    )
  ).rows;
  ...
  for (let i = 0; i < vuvales.length; i++) {
    ...
    for (let k = 0; k < kids; k++) {
      const boy = (i + k) % 2 === 0;
      const nm = boy
        ? MALE[(i + k + 1) % MALE.length]
        : FEMALE[(i + k + 2) % FEMALE.length];
      await add(
        v.id,
        `${nm} ${sur}`,
        boy ? "Son" : "Daughter",
        `${2006 + k * 3 + (i % 3)}-0${1 + (k % 8)}-20`
      );
      n++;
    }
