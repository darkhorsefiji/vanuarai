...
tables are _deprecated, not dropped_ — kept for rollback until the cutover (Phase 4).
...
| D2  | How an Outcome cascades           | **Shared outcome + rolling indicators.** An Outcome is authored once (at an apex node); every level reports the _same_ indicators against its own node, and figures roll UP via the existing `sum/avg/none` engine. One source of truth. |
...
3. **Task ↔ Intervention (hard rule).** A Task may attach to an intervention only while
   ...
   Enforced by DB trigger `enforce_task_intervention`. _API rule:_ an intervention with
...
best-fit and editable. The old per-KPI TAB-platform tag is thus _preserved_ as
