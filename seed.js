...
(async () => {
  await c.connect();
  const exists = await c.query("select 1 from villages where name=$1", ["Bagasau"]);
  if (exists.rowCount) { console.error("Bagasau already seeded — aborting to avoid duplicates."); await c.end(); process.exit(1); }
  ...
  async function node(axis, level, label, parent, opts = {}) {
    const r = await c.query(
      ...
      [axis, level, label, parent, opts.village_id ?? null, opts.vkb ?? null, opts.is_body ?? false, opts.is_platform ?? false]);
    return r.rows[0].id;
  }
  ...
  try {
    ...
    // Village tenant
    const vid = (await c.query("insert into villages(name) values($1) returning id", ["Bagasau"])).rows[0].id;
    ...
    let tokIdx = 0, counts = { yavusa: 0, mataqali: 0, tokatoka: 0, vuvale: 0 };
    for (const yl of ["A", "B"]) {
      const y = await node("traditional", "yavusa", `Yavusa ${yl}`, vanua, { village_id: vid });
      counts.yavusa++;
      for (const mi of [1, 2]) {
        const mlabel = `${yl}${mi}`;
        const m = await node("traditional", "mataqali", `Mataqali ${mlabel}`, y,
          { village_id: vid, is_body: true, vkb: `VKB-BAG-${mlabel}` });
        counts.mataqali++;
        for (const tl of ["a", "b"]) {
          const t = await node("traditional", "tokatoka", `Tokatoka ${mlabel}${tl}`, m, { village_id: vid });
          counts.tokatoka++;
          const n = VUVALE_COUNTS[tokIdx % VUVALE_COUNTS.length]; tokIdx++;
          for (let v = 1; v <= n; v++) {
            await node("traditional", "vuvale", `Vuvale ${mlabel}${tl}-${v}`, t, { village_id: vid });
            counts.vuvale++;
          }
        }
      }
