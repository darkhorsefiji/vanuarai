...
const NAMES = [
  ...
  "Litia Saukuru", "Joeli Naidu", "Watisoni Tora", "Mereani Tuwai", "Pita Vuki",
  "Asenaca Bola", "Inoke Tawake", "Salote Naivosa", "Meli Tuilevu", "Ana Vakuru",
  "Josefa Bose", "Timoci Rakai", "Kelera Vatu", "Epeli Naqica", "Akanisi Cava",
  "Waisake Dau", "Maraia Lewatu", "Semi Koroi", "Filipe Vodo", "Talei Rokobau",
];
...
(async () => {
  ...
  const holders = (await c.query(
    `select distinct u.id from users u join body_offices bo on bo.user_id=u.id
     where u.display_name <> 'Eugene Singh' order by u.id`)).rows;
  let i = 0;
  for (const h of holders) {
    const nm = NAMES[i % NAMES.length]; i++;
    await c.query(`update users set display_name=$1 where id=$2`, [nm, h.id]);
    await c.query(`update memberships set full_name_bc=$1 where user_id=$2`, [nm, h.id]);
  }
