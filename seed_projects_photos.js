...
function query(name) {
  ...
  if (n.includes("aquacultur") || n.includes("pond"))
    return "Fiji fishing fish";
...
(async () => {
  ...
  const mataqali = (
    await c.query(
      "select id from scope_nodes where level='mataqali' order by label limit 1"
    )
  ).rows[0].id;

  const extra = [
    {
      name: "Coastal Seawall",
      owner: v.village_node_id,
      budget: 1800000,
      prog: 35,
    },
  ...
  for (const np of extra) {
    if (
      (await c.query("select 1 from projects where name=$1", [np.name]))
        .rowCount
    )
      continue;
    const pot = (
      await c.query(
       ...
       values('temporary',$1,$2,$3,'sweep_to_parent',$4) returning id`,
        [np.owner, np.name, np.budget, "CUST-BAG-" + np.name.slice(0, 4)]
      )
    ).rows[0].id;
    await c.query(
      `insert into projects(owner_body_node_id,pot_id,name,budget_cents,physical_progress,status)
       values($1,$2,$3,$4,$5,'active')`,
      [np.owner, pot, np.name, np.budget, np.prog]
    );
  }

  const general = await commons(
    "Fiji village OR Pacific island village OR Fiji people",
    45
  );
  const projects = (
    await c.query("select id, name from projects order by name")
  ).rows;
  ...
  for (let i = 0; i < projects.length; i++) {
    ...
    let pi = 0;
    while (imgs.length < count && pi < pool.length) {
      if (!imgs.includes(pool[pi])) imgs.push(pool[pi]);
      pi++;
    }
  ...
  console.log(
    `Projects: ${projects.length}. Seeded ${total} Pacific/Fiji photos from Commons.`
  );
  await c.end();
})().catch((e) => {
