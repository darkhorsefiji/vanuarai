...
function Control({ it, value, onChange }) {
    ...
    return (
      <span className="styler-range">
        <input
          ...
          max={it.max}
          step={it.step || 1}
          value={parseFloat(value != null ? value : it.d)}
          onChange={(e) => onChange(e.target.value + (it.unit || ""))}
        />
  ...
  if (it.t === "toggle")
    return (
      <input
        type="checkbox"
        checked={(value || it.d) === it.on}
        onChange={(e) => onChange(e.target.checked ? it.on : "normal")}
      />
    );
  if (it.t === "font")
    return (
      <input
        ...
        onChange={(e) => {
          ...
          if (GOOGLE_FONTS.includes(e.target.value)) {
            ...
            onChange(fontStack(e.target.value));
          }
        }}
      />
    );
...
export function DevStyleToggle() {
  ...
  return (
    <button
      className={"hdrtool" + (s.on ? " on" : "")}
      onClick={s.toggle}
      title="DEV Style — click any element on the page to style it"
    >
      🎨
    </button>
  );
...
export function StyleModeProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const over = (e) => {
      ...
      if (hovered.current && hovered.current !== t?.el)
        hovered.current.classList.remove("style-hover");
      if (t && t.el !== selected.current) {
        t.el.classList.add("style-hover");
        hovered.current = t.el;
      }
    };
    const click = (e) => {
      const t = findTarget(e.target);
      ...
      if (!pinned.current)
        setPos({ x: e.clientX, y: e.clientY, dragged: false });
    };
    const esc = (e) => {
      if (e.key === "Escape") {
        setPop(null);
  ...
  const popStyle = pop
    ? {
        ...
        top:
          Math.max(8, Math.min(pos.dragged ? pos.y : pos.y + 14, H - 440)) +
          "px",
      }
    : null;
  ...
  return (
    <StyleCtx.Provider value={{ on, toggle, dev }}>
      ...
      {dev && on && !pop && (
        <div className="styler-hint">
          🎨 Click any element to style it · Esc closes the panel · toggle 🎨
          off to navigate
        </div>
      )}
      ...
      {g && (
        <div className="styler-pop" style={popStyle} ref={popRef}>
          <div
            className="styler-pop-head"
            onMouseDown={startDrag}
            title="Drag to move"
          >
            <b>
              <span className="styler-grip">⠿</span>
              {g.group}
            </b>
            <span>
              <button
          ...
          <div className="meta" style={{ marginTop: 8 }}>
            Changes save instantly · full editor on the Dev page
          </div>
        </div>
      )}
    </StyleCtx.Provider>
