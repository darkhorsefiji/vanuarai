...
export default function PlansEditor() {
  ...
  const load = () =>
    get("/plans").then((d) =>
      setRows(
        d.map((p) => ({
          ...p,
          priceStr: String(p.price_cents / 100),
          _dirty: false,
        }))
      )
    );
  ...
  const upd = (i, k, v) =>
    setRows((rs) =>
      rs.map((r, j) => (j === i ? { ...r, [k]: v, _dirty: true } : r))
    );
  ...
  async function saveRow(i) {
    const r = rows[i];
    if (!r.name.trim()) {
      setMsg("A plan name is required");
      return;
    }
    const body = {
      name: r.name.trim(),
  ...
  return (
    <div className="plansedit">
      ...
      {rows.map((r, i) => (
        <div
          className={"plansedit-row" + (r._dirty ? " dirty" : "")}
          key={r.id || "new" + i}
        >
          <input
            value={r.name}
          ...
          <input
            ...
            step="1"
            value={r.volume_mb}
            onChange={(e) => upd(i, "volume_mb", e.target.value)}
          />
          <select
            value={r.validity}
          ...
          <input
            ...
            step="0.5"
            value={r.priceStr}
            onChange={(e) => upd(i, "priceStr", e.target.value)}
          />
          ...
          <div className="plansedit-actions">
            <button className="btn" disabled={busy} onClick={() => saveRow(i)}>
              Save
            </button>
            <button className="mini" disabled={busy} onClick={() => delRow(i)}>
              Delete
            </button>
          </div>
        </div>
      ...
      <div className="plansedit-foot">
        ...
        <span className="plansedit-hint">
          Tip: 1 GB = 1024 MB. Price is in dollars.
        </span>
