...
const BLANK = { full_name: '', relationship: '', dob: '', dod: '' }
...
export default function VuvaleEditor() {
  ...
  async function savePerson(p) {
    const r = await fetch('/api/persons/' + p.id, {
      ...
      body: JSON.stringify({ full_name: p.full_name, relationship: p.relationship, date_of_birth: p.dob || null, date_of_death: p.dod || null }),
    })
  ...
  async function addPerson() {
    ...
    const r = await fetch('/api/persons', {
      ...
      body: JSON.stringify({ vuvale_node_id: sel, full_name: add.full_name, relationship: add.relationship, date_of_birth: add.dob || null, date_of_death: add.dod || null }),
    })
  ...
  return (
    <div>
      ...
      <table>
        <tbody>
          <tr><th>Name</th><th>Relationship</th><th>Born</th><th>Died</th><th></th></tr>
          {people.map((p, i) => (
            <tr key={p.id}>
