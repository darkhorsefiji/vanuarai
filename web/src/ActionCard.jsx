  ...
  useEffect(() => {
    if (autoOpen) {
      ...
      setF({
        ...blank,
        title: prefillTitle || "",
        description: `Addresses a shortfall under “${outcome.title}”.`,
        indicator_id: prefillIndicator || null,
      });
      onConsume && onConsume();
  ...
  async function create() {
    ...
    try {
      await send("POST", "/of/actions", {
        ...
        indicator_id: f.indicator_id || null,
        node_id: nodeId || null,
        target_due_date: f.target_due_date || null,
        actual_due_date: f.actual_due_date || null,
        status: f.status,
      });
      setF(blank);
      setShowCreate(false);
  ...
  return (
    <div className="card of-ic" ref={ref}>
      <div className="of-ic-head">
        <span className="of-ic-title">
          Interventions <span className="meta">({actions.length})</span>
        </span>
        {canEdit && (
          <button
            className={"mini " + (showCreate ? "" : "secondary")}
        ...
        <div className="of-ic-create">
          ...
          <div className="of-create-row">
            <label className="meta">
              ...
              <input
                type="date"
                value={f.target_due_date}
                onChange={(e) =>
                  setF({ ...f, target_due_date: e.target.value })
                }
              />
            </label>
            <select
      ...
      {actions.length === 0 ? (
        <p className="meta of-ic-empty">
          No interventions yet
          {canEdit ? " — use “＋ New”, or “Raise action” on a gap." : "."}
        </p>
      ) : (
        actions.map((a) => (
          <ActionRow
