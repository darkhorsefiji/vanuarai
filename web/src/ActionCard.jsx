...
export default function ActionCard({ outcome, nodeId, actions, childrenOf, govContacts, canEdit, onChange, autoOpen, prefillTitle, onConsume }) {
  const [showCreate, setShowCreate] = useState(false)
  const blank = { kind: 'intervention', title: '', description: '', target_due_date: '', status: 'not_started' }
  ...
  useEffect(() => {
    if (autoOpen) {
      setShowCreate(true)
      setF({ ...blank, title: prefillTitle || '', description: `Addresses a shortfall under “${outcome.title}”.` })
      onConsume && onConsume()
  ...
  async function create() {
    ...
    try {
      await send('POST', '/of/actions', {
        ...
        outcome_id: outcome.id, node_id: nodeId || null,
        target_due_date: f.target_due_date || null, status: f.status,
      })
    ...
    <div className="card of-ic" ref={ref}>
      ...
      {showCreate && canEdit && (
        <div className="of-ic-create">
          ...
          <div className="of-create-row">
            <label className="meta">due <input type="date" value={f.target_due_date} onChange={e => setF({ ...f, target_due_date: e.target.value })} /></label>
