  ...
  if (!check(user)) {
    return (
      <div className="card" style={{ marginTop: 20, maxWidth: 460 }}>
        <h3>{who} only</h3>
        <p className="sub">
          Sign in with a {who.toLowerCase()} account to access this page.
        </p>
      </div>
    );
  }
    ...
    <aside className={"sidebar" + (collapsed ? " collapsed" : "")}>
      ...
      {sideNav.map(([to, label, icon]) => (
        <NavLink
          key={to}
          to={to}
          title={dev ? label + " — drag to re-arrange" : label}
          className={({ isActive }) =>
            (isActive ? "active" : "") +
            (drag === to ? " nav-dragging" : "") +
            (over === to && drag && drag !== to ? " nav-over" : "")
          }
                ...
                <header className="top">
                  {logoOk ? (
                    <img
                      className="brandlogo"
                      src="/logo.png"
                      alt="RAIVANUA — Digital Village Enabler"
                  ...
                  <main>
                    <Routes>
                      ...
                      <Route
                        path="/dev"
                        element={
                          <RequireRole check={isDev} who="App Admins (DEV)">
                            <Dev />
                          </RequireRole>
                        }
                      />
