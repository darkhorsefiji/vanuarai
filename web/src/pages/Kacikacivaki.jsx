...
function PostBox({ channel, onPosted }) {
  ...
  async function post() {
    ...
    setMsg("");
    try {
      await send("POST", "/notices", {
        channel,
        body,
        expires_at: expires || null,
      });
      setBody("");
      setExpires(plusDays(14));
      setMsg("Posted ✓");
      onPosted();
    } catch (e) {
      setMsg("⚠ " + e.message);
  ...
  return (
    <div className="postbox">
      ...
      <div className="postbox-foot">
        <label className="postbox-expiry meta">
          Ends{" "}
          <input
            type="date"
            value={expires}
...
function NoticeCard({ n, canManage, onChanged }) {
  ...
  async function del() {
    ...
    try {
      await send("DELETE", "/notices/" + n.id);
      onChanged();
    } catch (e) {
      setMsg("⚠ " + e.message);
    }
    ...
    <div className={"card notice" + (active ? "" : " expired")}>
      <div className="notice-head">
        ...
        <span className="notice-date">
          ...
          <span className={"lchip " + (active ? "active" : "expired")}>
            {n.status}
          </span>
        </span>
      </div>
        ...
        <>
          ...
          {canManage && (
            <div className="notice-acts-row">
              ...
              <button className="mini danger" title="Delete" onClick={del}>
                🗑
              </button>
            </div>
          )}
...
function NoticeColumn({
  ...
  const activeList = notices
    ? notices.filter((n) => n.status === "Active")
    : null;
  const expiredList = notices
    ? notices.filter((n) => n.status !== "Active")
    : [];
  ...
  return (
    <div className="col">
      ...
      <EditableText id={subId} className="sub">
        {subtitle}
      </EditableText>
      {canPost ? (
        ...
        <p className="meta postlock">
          🔒{" "}
          {channel === "koro"
            ? "Only village officials can post official notices."
            : "Sign in to post."}
        </p>
      )}
      <div className="noticelist">
        ...
        {activeList && activeList.length === 0 && (
          <p className="meta">No active posts.</p>
        )}
  ...
  return (
    <>
      <div className="pagetop">
        <h1>Kacikacivaki</h1>
        <EditableText id="kacikacivaki.sub" className="sub">
          Village announcements — official notices from the Vanua and Government
          on the left, community postings from any member on the right.
        </EditableText>
      </div>

      <div className="cols cols-even">
        <NoticeColumn
          title="Nai Tukutuku in Koro"
          subtitle="Official — Vanua & Government hierarchies."
          subId="kacikacivaki.koro.sub"
          channel="koro"
          notices={koro}
          user={user}
          official={official}
        ...
        />
        <NoticeColumn
          title="Nai Tukutuku in Lewe ni Vanua"
          subtitle="Community — postings from any village member."
          subId="kacikacivaki.lewe.sub"
          channel="lewe"
          notices={lewe}
          user={user}
          official={official}
