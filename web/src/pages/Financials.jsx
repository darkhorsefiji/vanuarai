...
function BodyCell({ r }) {
  return r.level ? (
    <span className="body-cell">
      <LevelBadge level={r.level} />{" "}
      <span className="body-name" title={r.body}>
        {r.body}
      </span>
    </span>
  ) : (
...
function noneRow(cols) {
  return (
    <tr>
      <td colSpan={cols} className="meta">
        No records for this body.
      </td>
    </tr>
  );
}
  ...
  return (
    <div className={"who-block " + kind}>
      ...
      <div className="who-sub">
        <span className="who-entity">{who.entity}</span>
        {who.body ? " · " + who.body : ""}
      </div>
...
function WhoPopover({ tx }) {
  ...
  if (!tx.initiator && !tx.approver && !tx.donor)
  ...
  return (
    <span
      ...
      onFocus={place}
    >
      <span className="txwho-ic" aria-label="Who initiated / approved">
        ⓘ
      </span>
...
function ArchiveBtn({ path, label, onDone }) {
  async function go() {
    if (
      !window.confirm(
        `Void this ${label}? It will be hidden from all views but kept for records.`
      )
    )
      return;
    try {
  ...
  return (
    <button className="mini danger" title="Void (archive)" onClick={go}>
      🗑
    </button>
  );
}
  ...
  return (
    <div className="page-financials">
      <div className="pagetop">
        ...
        <EditableText id="financials.sub" className="sub">
          Member-tier view of the village's funds, transactions, assets and
          investments.
        </EditableText>
      </div>
      <BodyFilterBar
  ...
  return (
    <>
      ...
      <div className="totrow">
        ...
        <div className="tot">
          <b>{fjd(tout)}</b>Outflows
        </div>
        <div className="tot">
          <b>{fjd(tin - tout)}</b>Net balance
        </div>
      </div>
      <table>
        <tbody>
          ...
          {rows.map((r) => (
            <tr key={r.purpose}>
              ...
              <td style={{ textAlign: "right" }}>{fjd(r.tin)}</td>
              <td style={{ textAlign: "right" }}>{fjd(r.tout)}</td>
              <td style={{ textAlign: "right" }}>{fjd(r.tin - r.tout)}</td>
            </tr>
          ))}
...
function Transactions({ filter, canEdit, refresh, onArchive }) {
  ...
  return (
    <>
      ...
      <div className="totrow">
        <div className="tot">
          <b>{fjd(tin)}</b>Total in
        </div>
        <div className="tot">
          <b>{fjd(tout)}</b>Total out
        </div>
        <div className="tot">
          <b>{fjd(tin - tout)}</b>Net
        </div>
      </div>
        ...
        <tbody>
          {rows.map((t) => (
            <tr key={t.id}>
              ...
              <td>
                <span className="desc-cell" title={t.description}>
                  {t.description}
                </span>
              </td>
              <td>
              ...
              <td>
                <span
                  className={
                    "lchip " + (t.type === "In" ? "approved" : "declined")
                  }
                >
                  {t.type}
                </span>
...
function Assets({ filter, canEdit, refresh, onArchive }) {
  ...
  return (
    <>
      ...
      <div className="totrow">
        <div className="tot">
          <b>{rows.length}</b>Assets
        </div>
        <div className="tot">
          <b>{fjd(total)}</b>Total book value
        </div>
      </div>
...
function Investments({ filter, canEdit, refresh, onArchive }) {
  ...
  return (
    <>
      <p className="sub">
        Body funds placed in trusts, deposits and enterprise.
      </p>
      <div className="totrow">
        <div className="tot">
          <b>{fjd(invested)}</b>Invested
        </div>
        <div className="tot">
          <b>{fjd(current)}</b>Current value
        </div>
        <div className="tot">
          <b>
            {gain >= 0 ? "+" : ""}
            ...
            return (
              <tr key={i.id}>
                ...
                <td style={{ textAlign: "right" }}>
                  {fjd(i.current_value_cents)}
                </td>
                <td
                  style={{
                    textAlign: "right",
                    color: g >= 0 ? "#2f7a52" : "#a83b25",
                    fontWeight: 600,
                  }}
                >
                  {i.return_pct != null
