...
export default function Hierarchy() {
  ...
  const pbody = (p) => ({
    ...
    date_of_death: p.dod || null,
    is_deceased: !!p.is_deceased,
    is_owner: !!p.is_owner,
  });
  const updP = (i, k, v) =>
    setPeople((arr) => arr.map((p, idx) => (idx === i ? { ...p, [k]: v } : p)));
  async function savePerson(p) {
    const r = await fetch("/api/persons/" + p.id, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(pbody(p)),
    });
    setMsg(r.ok ? "Saved ✓" : "Error");
    if (r.ok) loadPeople(sel);
  }
  ...
  async function addPerson() {
    ...
    const r = await fetch("/api/persons", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ vuvale_node_id: sel, ...pbody(add) }),
    });
    if (r.ok) {
      setAdd(BLANK);
      loadPeople(sel);
    ...
    <>
      <div className="pagehead">
        <div>
          <h1>Vanua</h1>
          <p className="sub">
            <EditableText as="span" id="hierarchy.sub">
              Traditional lineage. Click a Vuvale to view its family.
            </EditableText>
            {edit ? " Editing on." : ""}
          </p>
        </div>
      ...
      <div className="cols cols-13">
        ...
        <aside className="col">
          ...
          {!vnode ? (
            <p className="sub">Select a Vuvale in the tree.</p>
          ) : (
                      ...
                      edit ? (
                        <tr key={p.id}>
                          <td>
                            <input
                              value={p.full_name || ""}
                              onChange={(e) =>
                                updP(i, "full_name", e.target.value)
                              }
                            />
                          </td>
                          <td>
                          ...
                          <td style={{ textAlign: "center" }}>
                            <input
                              ...
                              checked={!!p.is_owner}
                              onChange={(e) =>
                                updP(i, "is_owner", e.target.checked)
                              }
                        ...
                        <tr key={p.id}>
                          <td>
                            {p.full_name}
                            {p.is_owner && (
                              <span className="lchip itltb owner-chip">
                                Owner
                              </span>
                            )}
                          </td>
                      ...
                      <tr>
                        <td>
                          <input
                            ...
                            value={add.full_name}
                            onChange={(e) =>
                              setAdd({ ...add, full_name: e.target.value })
                            }
                          />
                        </td>
                        <td>
                          <GenderSelect
                            value={add.gender}
                            onChange={(e) =>
                              setAdd({ ...add, gender: e.target.value })
                            }
                          />
                        </td>
                        ...
                        <td>
                          <StatusSelect
                            value={add.is_deceased}
                            onChange={(e) =>
                              setAdd({
                                ...add,
                                is_deceased: e.target.value === "Deceased",
                              })
                            }
                          />
                        </td>
                        <td style={{ textAlign: "center" }}>
                          <input
                            type="checkbox"
                            checked={!!add.is_owner}
                            onChange={(e) =>
                              setAdd({ ...add, is_owner: e.target.checked })
                            }
                            title="Household owner"
                          />
                        </td>
                        <td>
                          <button className="mini" onClick={addPerson}>
                            + Add
                          </button>
                        </td>
                      </tr>
