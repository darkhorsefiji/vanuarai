...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    if (m) {
      editing ? m.dragging.enable() : m.dragging.disable();
    }
  }, [editing]);
  ...
  function cancel() {
    ...
    if (data.latitude != null) {
      ...
      markerObj.current?.setLatLng([data.latitude, data.longitude]);
    }
    setEditing(false);
    setStatus("");
  }
  ...
  const avgR = avg(res.map((r) => r.resource)),
    avgP = avg(res.map((r) => r.participation));
    ...
    <>
          ...
          <div>
            ...
            <p className="sub" style={{ color: "#eafaf7", margin: "6px 0 0" }}>
              {data.district && data.province
                ? `District of ${data.district} in the Province of ${data.province}`
                : data.district
                  ? `District of ${data.district}`
                  : data.province
            ...
            <div className="editrow">
              {editing ? (
                <>
                  ...
                  <span className="status" style={{ color: "#eafaf7" }}>
                    {status}
                  </span>
                </>
              ) : (
      ...
      <div className="cols">
        <div className="col">
          ...
          <EditableText id="profile.resources.sub" className="sub">
            Each sector is scored 0–5 for resource endowment (blue) and the
            village’s level of participation (green).
          </EditableText>
          <div className="totrow">
            <div className="tot">
              <b>{avgR.toFixed(1)}/5</b>Avg resource
            </div>
            <div className="tot">
              <b>{avgP.toFixed(1)}/5</b>Avg participation
            </div>
            <div className="tot">
              <b>{overall}/100</b>Overall index
            </div>
          </div>
          <div className="grid">
            {res.map((r) => (
        ...
        <aside className="col side sticky">
          ...
          {coords ? (
            <>
              ...
              <p className="meta">
                {editing ? (
                  <>
                    Click the map or drag the pin · {coords.lat.toFixed(4)},{" "}
                  ...
                  <>
                    <a
                      target="_blank"
                      rel="noreferrer"
                      href={`https://www.openstreetmap.org/?mlat=${coords.lat}&mlon=${coords.lon}#map=13/${coords.lat}/${coords.lon}`}
                    >
                      View larger map ↗
                    </a>{" "}
          ...
          {editing ? (
            <textarea
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, how_to_get_there: e.target.value }))
              }
            />
          ) : (
