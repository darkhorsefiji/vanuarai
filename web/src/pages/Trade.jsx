...
function ListingForm({ authorName, onPosted }) {
  ...
  useEffect(() => {
    if (!seller && authorName) setSeller(authorName);
  }, [authorName, seller]);

  const updItem = (i, k, v) =>
    setItems((arr) => arr.map((it, j) => (j === i ? { ...it, [k]: v } : it)));
  const itemsValid = items.every(
    (it) => it.produce.trim() && Number(it.qty) > 0
  );
  ...
  async function post() {
    ...
    try {
      await send("POST", "/trade-listings", {
        ...
        items: items.map((it) => ({
          produce: it.produce,
          qty_kg: Number(it.qty),
        })),
      });
      setItems([{ produce: "", qty: "" }]);
      setFrom("");
      setTo("");
      setMobile("");
      setSeller(authorName || "");
      setMsg("Posted ✓");
      onPosted();
    } catch (e) {
      setMsg("⚠ " + e.message);
    ...
    <div className="postbox">
      ...
      <div className="sellform">
        <input
          ...
          title="Posting on behalf of someone? Put their name here."
          style={{ flex: 1 }}
          value={seller}
      ...
      {items.map((it, i) => (
        <div className="sellform" style={{ marginTop: 7 }} key={i}>
          <input
            ...
            placeholder="Produce…"
            value={it.produce}
            onChange={(e) => updItem(i, "produce", e.target.value)}
          />
          <input
            type="number"
            min="0"
      ...
      <div className="sellform foot">
        <button
          className="mini"
          onClick={() => setItems((arr) => [...arr, { produce: "", qty: "" }])}
        >
          + Add Produce
        </button>
        <button
          ...
          title={
            canPost
              ? "Post listing"
              : "Produce, weight and both dates are required"
          }
        >
          Post
        </button>
  ...
  return (
    <div className="card tradecard">
      <div
        ...
        onClick={() => more > 0 && setExpanded((e) => !e)}
      ...
      <div className="meta seller-line">
        <span>
          {g.available_from || g.available_to
            ? `${g.available_from || "…"}${g.available_to ? ` → ${g.available_to}` : ""}`
            : ""}
        </span>
        {g.mobile && (
          <a className="seller-mob" href={"tel:" + g.mobile.replace(/\s/g, "")}>
            {g.mobile}
          </a>
        )}
      </div>
...
export default function Trade() {
  ...
  const mine = (g) =>
    user && (isOfficialRole(user) || g.created_by === user.id);
  async function delGroup(g) {
    if (
      !window.confirm(
        `Remove ${g.seller}'s listing (${g.items.length} item${g.items.length === 1 ? "" : "s"})?`
      )
    )
      return;
    try {
      await send("DELETE", "/trade-listing-groups/" + g.group_id);
      setMsg("Removed ✓");
      loadListings();
    } catch (e) {
      setMsg("⚠ " + e.message);
  ...
  const buyersShown = buyers
    ? buyers.filter(
        (b) =>
          !fB ||
          [b.name, b.buys, b.location].join(" ").toLowerCase().includes(fB)
      )
    : null;
  const fC = qContact.trim().toLowerCase();
  const contactsShown = contacts
    ? contacts.filter(
        (c) =>
          !fC ||
          [c.category, c.name, c.detail, c.location]
            .join(" ")
            .toLowerCase()
            .includes(fC)
      )
  ...
  return (
    <>
      <div className="pagetop">
        <h1>Trade</h1>
        <EditableText id="trade.sub" className="sub">
          Village enterprise and exchange — produce for sale, buyers, and the
          contacts that move things.
        </EditableText>
      </div>
      ...
      <div className="cols cols3">
        <div className="col">
          ...
          <EditableText id="trade.sellers.sub" className="sub">
            Produce available from the village.
          </EditableText>
          {user ? (
            <ListingForm
            ...
            <p className="meta postlock">
              🔒 Sign in to post what you have for sale.
            </p>
          )}
          {msg && <span className="status">{msg}</span>}
        ...
        <div className="col">
          ...
          <EditableText id="trade.buyers.sub" className="sub">
            Who's buying village produce.
          </EditableText>
          <input
          ...
          <div className="tradelist">
              ...
              buyersShown.map((b) => (
                <div className="card govcontact" key={b.id}>
                  ...
                  <div className="gc-rows">
                    <div>
                      <span>Location</span>
                      <em style={{ fontStyle: "normal" }}>{b.location}</em>
                    </div>
                    <div>
                      <span>Mobile</span>
                      {b.mobile ? (
                        <a href={"tel:" + b.mobile.replace(/\s/g, "")}>
                          {b.mobile}
                        </a>
                      ) : (
                        <em>—</em>
            ...
            {buyersShown && buyersShown.length === 0 && (
              <p className="meta">No buyers match.</p>
            )}
          </div>
        </div>
        ...
        <div className="col">
          ...
          <EditableText id="trade.contacts.sub" className="sub">
            Carriers, buses, boats, hostels, venues and schools.
          </EditableText>
          <input
            className="treesearch"
          ...
          <div className="tradelist">
              ...
              contactsShown.map((c) => (
                <div className="card govcontact" key={c.id}>
                  ...
                  <div className="gc-rows">
                    <div>
                      <span>Location</span>
                      <em style={{ fontStyle: "normal" }}>{c.location}</em>
                    </div>
                    <div>
                      <span>Mobile</span>
                      {c.mobile ? (
            ...
            {contactsShown && contactsShown.length === 0 && (
              <p className="meta">No contacts match.</p>
            )}
          </div>
        </div>
      </div>
