    ...
    <li>
      ...
      <span
        className={
          ...
          (sel === n.id ? " selected" : onPath ? " onpath" : "")
        }
        onClick={selectable ? () => onSelect(n.id) : undefined}
      >
        {n.label}
      </span>
      {edit && (
        <span className="rowacts">
          <button className="mini" onClick={() => onRename(n)} title="Rename">
            ✎
          </button>
          {child && (
            <button className="mini" onClick={() => onAdd(n, child)}>
              + {child}
            </button>
          )}
          <button
...
export function useNodes() {
  ...
  const addNode = async (parent, childName) => {
    ...
    if (!label) return;
    try {
      await send("POST", "/nodes", { parent_id: parent.id, label });
      setMsg("Added ✓");
    } catch (e) {
  ...
  const renameNode = async (nd) => {
    const label = window.prompt("Rename to:", nd.label);
    if (!label || label === nd.label) return;
    try {
      await send("PATCH", "/nodes/" + nd.id, { label });
      setMsg("Renamed ✓");
    } catch (e) {
  ...
  const delNode = async (nd, onDeleted) => {
    if (
      !window.confirm(
        `Archive "${nd.label}" and everything under and linked to it (efforts, contributions, transactions)?\n\nIt will be hidden from all views but kept for records. Members are not removed.`
      )
    )
      return;
    try {
      await send("DELETE", "/nodes/" + nd.id);
      setMsg("Archived ✓");
      if (onDeleted) onDeleted(nd);
    } catch (e) {
