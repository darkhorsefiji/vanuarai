...
export function RaciChips({ raci, onDel }) {
  ...
  return (
    <span className="of-raci">
      {raci.map((r) => (
        <span
          className="of-chip"
          key={r.id}
          title={r.raci + " · " + r.assignee_kind}
        >
  ...
  return (
    <div className="of-addraci">
      <select
        ...
        title="RACI role"
      >
        {Object.entries(RACI_ABBR).map(([k, v]) => (
      ...
      {kind === "gov_contact" ? (
        ...
        <input
          ...
          placeholder={
            kind === "agency" ? "e.g. Ministry of Agriculture" : "name or role"
          }
        />
      )}
      <button className="mini" onClick={add} disabled={!val}>
        Assign
      </button>
    </div>
  );
...
export function InterventionIndicators({ actionId, canEdit }) {
  ...
  const load = () =>
    get(`/of/actions/${actionId}/intervention-indicators`)
      .then(setRows)
      .catch(() => setRows([]));
  useEffect(() => {
    load();
  ...
  async function add() {
    ...
    try {
      await send("POST", "/of/intervention-indicators", {
        action_id: actionId,
        ...f,
      });
      setF({ name: "", unit: "", target_value: "", actual_value: "" });
      load();
    } catch (e) {
      alert(e.message);
    }
  ...
  async function del(id) {
    try {
      await send("DELETE", "/of/intervention-indicators/" + id);
      load();
    } catch (e) {
      alert(e.message);
    }
  ...
  return (
    <div className="of-ii">
      <div className="of-sub-h">
        Intervention indicators <span className="meta">(short-term)</span>
      </div>
      {rows.length === 0 && <p className="meta">None yet.</p>}
      {rows.map((r) => (
        <div className="of-ii-row" key={r.id}>
          <span>
            {r.name} {r.unit ? <span className="meta">{r.unit}</span> : null}
          </span>
          <span className="sc-kpi-val">
            {Number(r.actual_value).toLocaleString()} /{" "}
            {Number(r.target_value).toLocaleString()}
          </span>
          {canEdit && (
            <button
              className="of-chip-x"
      ...
      {canEdit && (
        <div className="of-ii-add">
          ...
          <input
            ...
            onChange={(e) => setF({ ...f, actual_value: e.target.value })}
            placeholder="actual"
          />
          <input
            type="number"
            value={f.target_value}
            onChange={(e) => setF({ ...f, target_value: e.target.value })}
            placeholder="target"
          />
          <button className="mini" onClick={add} disabled={!f.name.trim()}>
            Add
          </button>
        </div>
      )}
...
export function AddTask({ parentId, parentOutcomeId, onDone }) {
  ...
  async function add() {
    ...
    try {
      await send("POST", "/of/actions", {
        ...
        outcome_id: parentOutcomeId || null,
      });
      setT("");
      onDone();
    } catch (e) {
  ...
  return (
    <div className="of-addtask">
      ...
      />
      <button className="mini" onClick={add} disabled={!t.trim()}>
        Add task
      </button>
    </div>
  );
...
export function Challenges({ action, canEdit }) {
  ...
  const load = () =>
    get("/of/challenges?action=" + action.id)
      .then(setList)
      .catch(() => setList([]));
  useEffect(() => {
    load();
  ...
  async function raise() {
    if (!desc.trim()) return;
    try {
      await send("POST", "/of/challenges", {
        action_id: action.id,
        description: desc.trim(),
        target_due_date: due || null,
      });
      setDesc("");
      setDue("");
      setAdding(false);
  ...
  async function setStatus(c, status) {
    try {
      await send("PATCH", "/of/challenges/" + c.id, { status });
      load();
    } catch (e) {
      alert(e.message);
    }
  ...
  return (
    <div className="of-challenges">
      <div className="of-sub-h">
        Challenges <span className="meta">(impediments)</span>
      </div>
      ...
      {list.map((c) => (
        <div className="of-ch-row" key={c.id}>
          ...
          {c.target_due_date && (
            <span className="meta">due {c.target_due_date.slice(0, 10)}</span>
          )}
          <select
            className={"of-status of-s-" + c.status}
            value={c.status}
        ...
        (adding ? (
          <div className="of-ch-add">
            <input
              ...
              placeholder="What is blocking result achievement?"
            />
            ...
            <button className="mini" onClick={raise} disabled={!desc.trim()}>
              Log
            </button>
            <button className="mini secondary" onClick={() => setAdding(false)}>
              Cancel
            </button>
          </div>
        ) : (
          <button className="mini secondary" onClick={() => setAdding(true)}>
            ＋ Log a challenge
          </button>
        ))}
    </div>
...
export function ActionRow({
  ...
  async function setStatus(status) {
    try {
      await send("PATCH", "/of/actions/" + a.id, { status });
      onChange();
    } catch (e) {
      alert(e.message);
    }
  ...
  async function del() {
    ...
    try {
      await send("DELETE", "/of/actions/" + a.id);
      onChange();
    } catch (e) {
      alert(e.message);
    }
  }
  async function delRaci(r) {
    try {
      await send("DELETE", "/of/raci/" + r.id);
      onChange();
    } catch (e) {
      alert(e.message);
    }
  }
  async function setDue(field, val) {
    try {
      await send("PATCH", "/of/actions/" + a.id, { [field]: val || null });
      onChange();
    } catch (e) {
      alert(e.message);
    }
    ...
    <>
      <div className={"of-action" + (nested ? " nested" : "")}>
        ...
        >
          <span className="of-idcol">
            <span className={"of-kind of-k-" + a.kind}>
              {KIND_LABEL[a.kind]}
            </span>
          ...
          <span className="of-titlecol">
            ...
            <span className="of-dues">
              <span className="meta">
                Target due: <b>{dOnly(a.target_due_date) || "—"}</b>
              </span>
              <span className="meta">
                Actual due: <b>{dOnly(a.actual_due_date) || "—"}</b>
              </span>
            </span>
          </span>
        </div>
        ...
        <select
          className={"of-status of-s-" + a.status}
          value={a.status}
        ...
        {canEdit && (
          <button className="mini danger" onClick={del} title="Delete">
            🗑
          </button>
        )}
      </div>
      {open && (
        <div className="of-drawer">
          ...
          {canEdit && (
            <div className="of-dueedit">
              <label className="meta">
                ...
                <input
                  type="date"
                  value={dOnly(a.target_due_date)}
                  onChange={(e) => setDue("target_due_date", e.target.value)}
                />
              </label>
              <label className="meta">
                Actual due{" "}
                <input
                  type="date"
                  value={dOnly(a.actual_due_date)}
                  onChange={(e) => setDue("actual_due_date", e.target.value)}
                />
              </label>
            </div>
          )}
          ...
          {(a.kind === "intervention" || a.kind === "project") && (
            <InterventionIndicators actionId={a.id} canEdit={canEdit} />
          )}
          {a.kind === "intervention" && (
            <div className="of-tasks">
              ...
              {canEdit && a.status !== "completed" && (
                <AddTask
                  parentId={a.id}
                  parentOutcomeId={a.outcome_id}
                  onDone={onChange}
                />
              )}
              {canEdit && a.status === "completed" && (
                <p className="meta">Completed — no new tasks can be added.</p>
              )}
            </div>
          )}
