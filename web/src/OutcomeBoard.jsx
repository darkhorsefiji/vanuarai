...
const EDITABLE_LEVELS = ['mataqali', 'tokatoka', 'vuvale', 'district', 'village']
...
export default function OutcomeBoard() {
  ...
  useEffect(() => { get('/of/taxonomies').then(setTax).catch(() => {}); get('/gov-contacts').then(setGovContacts).catch(() => {}) }, [])
  const qs = `axis=${axis}${focus ? '&focus=' + focus : ''}${pillar ? '&pillar=' + pillar : ''}${isic ? '&isic=' + isic : ''}`
  useEffect(() => { setOutcomes(null); get('/of/outcomes?' + qs).then(setOutcomes).catch(() => setOutcomes([])) }, [qs, refresh])
  useEffect(() => { setSc(null); get(`/of/scorecard?${qs}${level ? '&level=' + level : ''}`).then(setSc).catch(() => setSc({ rows: [], levels: [] })) }, [qs, level, refresh])
  const loadActions = () => get('/of/actions').then(setActions).catch(() => setActions([]))
  useEffect(() => { loadActions() }, [refresh])
  ...
  useEffect(() => {
    if (editing && nodeList.length === 0) {
      ...
      get('/of/indicators').then(setIndicators).catch(() => {})
    }
  }, [editing, nodeList.length])
  const loadOwn = id => get('/of/node/' + id).then(setOwn).catch(() => setOwn([]))
  useEffect(() => { if (pickNode) { setOwn(null); loadOwn(pickNode) } }, [pickNode])

  async function upsert(indId, tgt, act) {
    await send('POST', '/of/measurements', { indicator_id: indId, node_id: pickNode, target_value: tgt, actual_value: act })
    loadOwn(pickNode); bump()
  }
  const saveRow = (r, tgt, act) => upsert(r.indicator_id, tgt, act).then(() => setMsg('Saved ✓')).catch(e => setMsg('⚠ ' + e.message))
  async function delRow(r) {
    if (!window.confirm('Remove this measurement?')) return
    try { await send('DELETE', '/of/measurements/' + r.id); setMsg('Removed ✓'); loadOwn(pickNode); bump() }
    catch (e) { setMsg('⚠ ' + e.message) }
  }
  function addRow() {
    if (!addInd) return
    upsert(addInd, Number(addTgt) || 0, Number(addAct) || 0)
      .then(() => { setAddInd(''); setAddTgt(''); setAddAct(''); setMsg('Added ✓') })
      .catch(e => setMsg('⚠ ' + e.message))
  }
  ...
  for (const r of sc.rows) {
    const node = (byNode[r.node_id] ||= { id: r.node_id, label: r.node_label, outcomes: {} })
  ...
  const topFor = oid => actions.filter(a => a.outcome_id === oid && (a.kind !== 'task' || !a.parent_intervention_id))
  const childrenOf = id => actions.filter(a => a.parent_intervention_id === id)
  ...
  return (
    <div className="scorecard of-board">
      ...
      <div className="finfilter">
        ...
        <button className={'fchip' + (axis === 'traditional' ? ' active' : '')} onClick={() => { setAxis('traditional'); setLevel('') }}>Vanua</button>
        <button className={'fchip' + (axis === 'government' ? ' active' : '')} onClick={() => { setAxis('government'); setLevel('') }}>Government</button>
        <span className="finfilter-lbl" style={{ marginLeft: 14 }}>Level</span>
        {sc.levels.map(l => (
      ...
      <div className="finfilter of-classfilter">
        ...
        <button className={'fchip' + (focus === '' ? ' active' : '')} onClick={() => setFocus('')}>All</button>
        {tax.focus_areas.map(f => (
          <button key={f.id} className={'fchip' + (focus === f.id ? ' active' : '')} style={focus === f.id ? { borderColor: f.accent } : null} onClick={() => setFocus(f.id)}>{f.label}</button>
        ))}
      </div>
      ...
      <div className="finfilter of-classfilter">
        ...
        <label className="of-isicsel" style={{ marginLeft: 14 }}>ISIC&nbsp;
          <select value={isic} onChange={e => setIsic(e.target.value)}>
            ...
            {tax.isic.map(s => <option key={s.code} value={s.code}>{s.code} · {s.title}</option>)}
          </select>
        </label>
      </div>
        ...
        <div className="card sc-editor">
          ...
          {pickNode && (own == null ? <p className="loading">Loading…</p> : (
            <table className="sc-ed-table">
              <tbody>
                <tr><th>Indicator</th><th>Actual</th><th>Target</th><th></th></tr>
                {own.map(r => <EditRow key={r.id} row={r} onSave={saveRow} onDel={delRow} />)}
                <tr className="sc-ed-add">
                  <td>
                    <select value={addInd} onChange={e => setAddInd(e.target.value)}>
                      <option value="">+ Add an indicator…</option>
                      {indicators.filter(k => !own.some(o => o.indicator_id === k.id)).map(k => <option key={k.id} value={k.id}>{k.outcome_title} · {k.name}</option>)}
                    </select>
                  </td>
                  ...
                  <td><button className="mini" onClick={addRow} disabled={!addInd}>Add</button></td>
                </tr>
              </tbody>
            </table>
          ))}
          <p className="meta sc-ed-hint">Enter each node's own Actual/Target; figures roll up to the parent levels automatically (sum / avg / local).</p>
        </div>
      )}

      {/* ── Per-Outcome rows: scorecard (left) · interventions (right) ──────── */}
      {nodes.length === 0
        ? <p className="meta">No measurements recorded at this level yet{focus || pillar || isic ? ' for this filter' : ''}. {canEdit ? 'Use “✎ Enter data”.' : ''}</p>
        : nodes.map(nd => (
          <div key={nd.id}>
                        ...
                        <div className="sc-kpi" key={j}>
                          <div className="sc-kpi-top">
                            ...
                            <span className="sc-kpi-val">
                              {fmt(k.actual, k.unit)} / {fmt(k.target, k.unit)}{k.unit && k.unit !== 'FJD' ? ' ' + k.unit : ''} · {pct}%
                              <span className={'of-var ' + (varc < 0 ? 'neg' : 'pos')}>{varc < 0 ? '▾' : '▴'} {fmt(Math.abs(varc), k.unit)}</span>
                            </span>
                          </div>
