...
export function setToken(t) {
  if (t) localStorage.setItem("vr_token", t);
  else localStorage.removeItem("vr_token");
}
...
export async function send(method, path, body) {
  ...
  try {
    ...
    throw new Error(
      "Server unreachable — your input is kept; please try again shortly."
    );
  }
  if (!r.ok) {
    let msg;
...
export const fjd = (c) =>
  "$" +
  (Number(c) / 100).toLocaleString(undefined, { minimumFractionDigits: 0 });
