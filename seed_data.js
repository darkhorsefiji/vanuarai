...
(async () => {
  ...
  if ((await c.query("select 1 from users limit 1")).rowCount) {
    console.error("Users already exist — aborting to avoid duplicates.");
    await c.end();
    process.exit(1);
  }
  ...
  const vid = village.id,
    govVillage = village.village_node_id;
  const nodes = await all(
    "select id,axis,level,label,parent_id from scope_nodes"
  );
  const byId = Object.fromEntries(nodes.map((n) => [n.id, n]));
  const mataqalis = nodes
    .filter((n) => n.level === "mataqali")
    .sort((a, b) => a.label.localeCompare(b.label));
  const vuvales = nodes.filter((n) => n.level === "vuvale");
  const ancestorAtLevel = (n, lvl) => {
    let x = n;
    while (x && byId[x.parent_id]) {
      x = byId[x.parent_id];
      if (x.level === lvl) return x;
    }
    return null;
  };
  const vuvaleByMataqali = {}; // mataqaliId -> [vuvale...]
  for (const v of vuvales) {
    const m = ancestorAtLevel(v, "mataqali");
    if (m) (vuvaleByMataqali[m.id] ??= []).push(v);
  }
  ...
  const mkUser = async (name, email, appAdmin = false) =>
    (
      await one(
        `insert into users(google_sub,email,display_name,is_app_admin) values($1,$2,$3,$4) returning id`,
        ["seed:" + email, email, name, appAdmin]
      )
    ).id;
      ...
      await one(
       ...
       values($1,$2,$3,'approved',$4,$5,$6,'Fiji',$7,now()) returning id`,
        [
          ...
          vuvaleId,
          name(uid),
          "BC-BAG-" + String(++bcSeq).padStart(4, "0"),
          isOff ? "TIN-BAG-" + String(++tinSeq).padStart(3, "0") : null,
        ]
      )
    ).id;
  };
  ...
  const user = async (display, email, appAdmin = false) => {
    const id = await mkUser(display, email, appAdmin);
    names[id] = display;
    return id;
  };
  const office = (bodyNode, kind, uid, fallback = false) =>
    c.query(
      `insert into body_offices(body_node_id,office,user_id,is_fallback) values($1,$2,$3,$4)`,
      [bodyNode, kind, uid, fallback]
    );
  ...
  try {
    ...
    const soqo = [];
    for (const s of [
      ["Soqosoqo Vakamarama (Women)", "wmn"],
      ["Soqosoqo Tabagone (Youth)", "yth"],
    ]) {
      const id = (
        await one(
    ...
    const admin = await user(
      "Bagasau Village Administrator",
      "admin@bagasau.test",
      false
    );
    await mkMembership(admin, "official", someVuvale);
    await office(govVillage, "village_admin", admin);
    const vHead = await user("Turaga ni Koro (Bagasau)", "tnk@bagasau.test");
    await mkMembership(vHead, "official", someVuvale);
    await office(govVillage, "head", vHead);
    const vSec = await user(
      "Vunivola (Village)",
      "vunivola.village@bagasau.test"
    );
    await mkMembership(vSec, "official", someVuvale);
    await office(govVillage, "vunivola", vSec);
    const vTre = await user(
      "Dau ni yau (Village)",
      "dauniyau.village@bagasau.test"
    );
    await mkMembership(vTre, "official", someVuvale);
    await office(govVillage, "dauniyau", vTre);
    await office(govVillage, "head", vSec, true); // fallback checker = vunivola (so head can initiate)
    bodyOfficers[govVillage] = {
      head: vHead,
      vunivola: vSec,
      dauniyau: vTre,
      admin,
    ...
    for (const m of mataqalis) {
      ...
      const head = await user(
        `Turaga ni Mataqali ${m.label.replace("Mataqali ", "")}`,
        `tnm.${m.label.slice(-2)}@bagasau.test`
      );
      await mkMembership(head, "official", vv);
      await office(m.id, "head", head);
      const sec = await user(
        `Vunivola (${m.label})`,
        `vunivola.${m.label.slice(-2)}@bagasau.test`
      );
      await mkMembership(sec, "official", vv);
      await office(m.id, "vunivola", sec);
      const tre = await user(
        `Dau ni yau (${m.label})`,
        `dauniyau.${m.label.slice(-2)}@bagasau.test`
      );
      await mkMembership(tre, "official", vv);
      await office(m.id, "dauniyau", tre);
    ...
    // Soqosoqo bodies
    for (const s of soqo) {
      const liuliu = await user(
        `Liuliu (${s.label})`,
        `liuliu.${s.key}@bagasau.test`
      );
      await mkMembership(liuliu, "official", someVuvale);
      await office(s.id, "liuliu", liuliu);
      const sec = await user(
        `Vunivola (${s.label})`,
        `vunivola.${s.key}@bagasau.test`
      );
      await mkMembership(sec, "official", someVuvale);
      await office(s.id, "vunivola", sec);
      const tre = await user(
        `Dau ni yau (${s.label})`,
        `dauniyau.${s.key}@bagasau.test`
      );
      await mkMembership(tre, "official", someVuvale);
      await office(s.id, "dauniyau", tre);
    ...
    for (let i = 1; i <= 8; i++) {
      ...
      const mem = await mkMembership(
        u,
        "member",
        vuvales[i % vuvales.length].id
      );
    ...
    const plans = [
      ["Daily", 500, "daily", 200],
      ["Weekly", 2048, "weekly", 800],
      ["Fortnightly", 4096, "fortnightly", 1500],
      ["Monthly", 10240, "monthly", 3000],
    ];
    for (const [pn, mb, win, cents] of plans)
      await c.query(
        `insert into plans(village_id,name,volume_mb,validity,price_cents) values($1,$2,$3,$4,$5)`,
        [vid, pn, mb, win, cents]
      );

    // ---- minutes + resolutions ----
    const mkMinutes = async (
      nodeId,
      title,
      date,
      createdBy,
      ...
      const m = await one(
        `insert into minutes(classification_node_id,title,meeting_date,body_text,created_by)
        values($1,$2,$3,$4,$5) returning id`,
        [nodeId, title, date, "Seed minutes — " + title, createdBy]
      );
      const r = await one(
        `insert into resolutions(minutes_id,ref_label,summary) values($1,$2,$3) returning id`,
    ...
    for (const d of quarters) {
      qi++;
      const r = await mkMinutes(
        govVillage,
        `Village Meeting — Q${((qi - 1) % 4) + 1} ${d.slice(0, 4)}`,
        d,
        vSec,
        `VR-${d.slice(0, 4)}/${((qi - 1) % 4) + 1}`,
        "Adopted quarterly report and budget."
      );
      villageRes.push(r);
    }
    const mataqaliRes = {};
    for (const m of mataqalis) {
      const r = await mkMinutes(
        m.id,
        `${m.label} Meeting — 2026 Q1`,
        "2026-03-31",
        bodyOfficers[m.id].vunivola,
        `MR-${m.label.slice(-2)}-2026/1`,
        "Approved land allocation and lease matters."
      );
      mataqaliRes[m.id] = r;
    }
    ...
    const mkPot = async (ownerNode, purpose, goalCents, custRef) =>
      (
        ...
        values('temporary',$1,$2,$3,'sweep_to_parent',$4) returning id`,
          [ownerNode, purpose, goalCents, custRef]
        )
      ).id;
    ...
    const projDefs = [
      ...
      {
        owner: govVillage,
        name: "Bagasau Church Restoration",
        budget: 4000000,
        goal: 4000000,
        prog: 10,
      },
      ...
      {
        owner: mataqalis[0].id,
        name: "Mataqali Farm Access Road",
        budget: 1200000,
        goal: 1200000,
        prog: 20,
      },
    ...
    for (const pd of projDefs) {
      const pot = await mkPot(
        pd.owner,
        pd.name,
        pd.goal,
        "CUST-BAG-" + (projects.length + 1)
      );
      ...
      for (let k = 1; k <= 2; k++)
        await c.query(
          `insert into project_photos(project_id,image_ref,caption) values($1,$2,$3)`,
          [
            proj.id,
            `media/seed/${proj.id}-${k}.jpg`,
            `${pd.name} progress photo ${k}`,
          ]
        );
    ...
    const appr = await one(
      ...
      [
        ...
        vTre,
        vHead,
        villageRes[4].resolution,
        "media/seed/receipt-hall.jpg",
    ...
    await c.query(
      ...
      [
        ...
        vTre,
        vHead,
        villageRes[5].resolution,
        "media/seed/quote-water.jpg",
