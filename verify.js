...
(async () => {
  ...
  if ((await c.query("select 1 from users limit 1")).rowCount) {
    const plans = await c.query("select name, volume_mb, validity, price_cents from plans order by price_cents");
    console.log("\nPlans:");
    plans.rows.forEach(p => console.log(`  ${p.name.padEnd(12)} ${(p.volume_mb + "MB").padEnd(9)} ${p.validity.padEnd(12)} FJD ${(p.price_cents / 100).toFixed(2)}`));
    ...
    proj.rows.forEach(r => console.log(
    ...
    const appr = await c.query("select action, status, count(*)::int n from approvals group by action,status order by status");
    console.log("\nMaker-checker approvals:");
    appr.rows.forEach(r => console.log(`  ${r.action.padEnd(14)} ${r.status.padEnd(10)} ${r.n}`));
  }
