...
(async () => {
  ...
  const offices = (await c.query(
    `select bo.id, bo.user_id, bo.office, sn.axis from body_offices bo
     join scope_nodes sn on sn.id=bo.body_node_id where bo.active=true`)).rows;
  const txs = (await c.query(
    `select t.id, t.tx_date, sn.axis from fin_transactions t
     left join scope_nodes sn on sn.id=t.classification_node_id order by t.tx_date, t.id`)).rows;

  // per-axis pools: initiators (treasurer first), approvers (head/liuliu first)
  const pool = (axis, kinds) => {
    for (const k of kinds) { const m = offices.filter(o => o.axis === axis && o.office === k); if (m.length) return m; }
  ...
  for (const t of txs) {
    ...
    const approver = pick("a:" + axis, pool(axis, ["head", "liuliu", "vunivola"]), initiator?.user_id);
